...
export default class UIBase extends UI
{
   constructor (width?: number, height?: number)
   {
      ...
      if (width && isNaN(width) && String(width).indexOf('%') != -1) {
          ...
          this.setting.widthPct = parseFloat(String(width).replace('%', '')) * 0.01;
      } else {
      ...
      {
          this.setting.heightPct = parseFloat(String(height).replace('%', '')) * 0.01;
      } else {
    ...
    * @private
    */
   private updatesettings (updateChildren: boolean, updateParent: boolean)
   {
   ...
   initDraggable()
   {
       if (!this.dragInitialized) {
           ...
           this.drag.onDragStart = function (e: PIXI.interaction.InteractionEvent)
           {
               ...
               if (!this.dragging && added)
               {
                   ...
                   //H containerStart.copy(this.container.position);
                   //H copy does not exist on type Point
                   containerStart.x = this.container.position.x;
                   containerStart.y = this.container.position.y;
           ...
           this.drag.onDragMove = function (e:PIXI.interaction.InteractionEvent, offset: PIXI.Point)
           {
               if (this.dragging) {
           ...
           this.drag.onDragEnd = function (e: PIXI.interaction.InteractionEvent) {
               if (this.dragging) {
                   this.dragging = false;
   ...
   initDroppable()
   {
       if (!this.dropInitialized) {
           ...
           this.onDrop = function (event: PIXI.interaction.InteractionEvent) {
